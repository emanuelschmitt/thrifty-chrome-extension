...
function getMinPrice($: CheerioAPI): number | null {
  ...
  $('div[data-testid="price"]').each((index, element) => {
...
function getNumberOfResults($: CheerioAPI): number {
  const summaryText = $('div[data-q^="section-top-and-left-and-middle"] > div > h1').text()
  const regex = /(\d+) ads/
...
export function parseGumtree(html: string): SearchResult | null {
  const $ = load(html)
